...
async def help_callback(callback: types.CallbackQuery, user: dict = None):
    await callback.message.edit_text(
        ...
        "🤖 Доступные модели:\n"
        "• GPT-4 - самая мощная модель, отлично подходит для сложных задач\n"
        "• Claude 3 - специализируется на длинных диалогах и анализе\n"
        "• Together - быстрая модель для простых запросов\n\n"
        ...
        "2. Пополните баланс\n"
        "3. Отправляйте текстовые сообщения или включите режим генерации изображений\n\n"
        "Выберите действие:",
        reply_markup=get_start_keyboard(user.get("image_mode", False)),
    )
